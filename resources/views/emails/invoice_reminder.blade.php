        ...
        <table class="main" width="100%">
            <tr>
                <td class="header">
                    ...
                    <p>Peringatan Tagihan Pembayaran</p>
                </td>
            </tr>
            <tr>
                <td class="content">
                    <div class="badge" style="background-color: #fffbeb; color: #d97706; border-color: #fde68a;">
                        <span class="badge-dot" style="background-color: #f59e0b;"></span>Menunggu Pembayaran
                    </div>
                    <h2>Halo, {{ $user->name ?? 'Admin' }}!</h2>
                    <div class="message-card">
                        <p>Kami ingin mengingatkan bahwa tagihan layanan company profile {{ $tenant->name ?? 'Tenant' }} akan jatuh tempo dalam <strong>{{ $daysLeft ?? 0 }} hari</strong>. Mohon segera selesaikan pembayaran untuk menghindari penangguhan layanan.</p>
                        <br>
                        <p>Total Tagihan: <strong>Rp {{ number_format($payment->amount_due ?? 0, 0, ',', '.') }}</strong></p>
                    </div>
                    <p class="instructions">Silakan klik tombol di bawah ini untuk menuju dashboard dan mengunggah bukti pembayaran Anda.</p>
                    <div class="cta-container">
                        <a href="{{ url('/admin') }}" class="btn" target="_blank">Bayar Sekarang</a>
                    </div>
